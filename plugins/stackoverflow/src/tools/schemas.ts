...
// biome-ignore lint/suspicious/noExplicitAny: API returns untyped JSON, mappers defensively handle all fields
...
export const mapQuestion = (q: RawData) => ({
  ...
  creation_date: unixToIso(q.creation_date),
  last_activity_date: unixToIso(q.last_activity_date),
  link: q.link ?? '',
  owner_display_name: q.owner?.display_name ?? '',
  owner_reputation: q.owner?.reputation ?? 0,
  owner_user_id: q.owner?.user_id ?? 0,
...
export const mapAnswer = (a: RawData) => ({
  ...
  creation_date: unixToIso(a.creation_date),
  last_activity_date: unixToIso(a.last_activity_date),
  owner_display_name: a.owner?.display_name ?? '',
  owner_reputation: a.owner?.reputation ?? 0,
  owner_user_id: a.owner?.user_id ?? 0,
});
...
export const mapComment = (c: RawData) => ({
  ...
  body: c.body ?? '',
  score: c.score ?? 0,
  creation_date: unixToIso(c.creation_date),
  owner_display_name: c.owner?.display_name ?? '',
  owner_user_id: c.owner?.user_id ?? 0,
});
...
export const mapUser = (u: RawData) => ({
  ...
  creation_date: unixToIso(u.creation_date),
  last_access_date: unixToIso(u.last_access_date),
  location: u.location ?? '',
  website_url: u.website_url ?? '',
  badge_gold: u.badge_counts?.gold ?? 0,
  badge_silver: u.badge_counts?.silver ?? 0,
  badge_bronze: u.badge_counts?.bronze ?? 0,
});
...
export const mapSearchExcerpt = (s: RawData) => ({
  ...
  creation_date: unixToIso(s.creation_date),
});
