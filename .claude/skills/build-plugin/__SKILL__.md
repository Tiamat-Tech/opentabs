...
28. **Write operations may require undocumented cryptographic send tokens** — Some web apps (e.g., Gmail's sync API) accept draft creation and label changes but require an additional cryptographic payload (generated by obfuscated JS) to finalize write operations like sending email. If a write operation returns 200 but the action doesn't take effect, the API likely requires an additional authorization step that cannot be replicated. In these cases, omit the tool rather than shipping one that silently fails. Document the limitation clearly.
