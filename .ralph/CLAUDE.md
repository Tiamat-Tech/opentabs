...
- **Worktrees are fully set up before the container starts.** Each worktree gets `bun install` (own `node_modules/`), `bun run build` (fresh `dist/` for all platform packages), and the `plugins/e2e-test` plugin is installed and built. This runs on the host (using the host's bun cache for fast installs). The resulting files are bind-mounted into the container.
...
- **Claude CLI credentials.** The container mounts `~/.claude/` and `~/.claude.json` read-only for auth. Environment variables from `~/.claude/settings.json` (e.g., `ANTHROPIC_BASE_URL`) are extracted and passed as container env vars. The host's `ANTHROPIC_*` env vars are also forwarded.
