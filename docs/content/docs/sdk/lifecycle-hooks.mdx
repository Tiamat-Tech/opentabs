...
Lifecycle hooks let plugins respond to adapter registration, navigation, and tool execution events. All hooks are optional methods on the `OpenTabsPlugin` base class — implement only the ones you need.
...
```typescript
class MyPlugin extends OpenTabsPlugin {
  onToolInvocationStart(toolName: string) {
...
```typescript
class MyPlugin extends OpenTabsPlugin {
  onToolInvocationEnd(toolName: string, success: boolean, durationMs: number) {
    if (!success) {
