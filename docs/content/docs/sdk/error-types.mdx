...
`ToolError` is the structured error type for tool handlers. When a tool throws a `ToolError`, the platform propagates its metadata (code, category, retryable, retryAfterMs) all the way to the AI agent, enabling intelligent retry decisions.
...
```typescript
static auth(message: string): ToolError;
// Sets: code='AUTH_ERROR', category='auth', retryable=false
```
...
```typescript
static rateLimited(message: string, retryAfterMs?: number): ToolError;
// Sets: code='RATE_LIMITED', category='rate_limit', retryable=true
```
...
```typescript
static validation(message: string): ToolError;
// Sets: code='VALIDATION_ERROR', category='validation', retryable=false
```
...
```typescript
static timeout(message: string): ToolError;
// Sets: code='TIMEOUT', category='timeout', retryable=true
```
...
```typescript
static internal(message: string): ToolError;
// Sets: code='INTERNAL_ERROR', category='internal', retryable=false
```
