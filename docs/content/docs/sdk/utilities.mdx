...
import {
  ...
  fetchFromPage, fetchJSON, postJSON,
  getLocalStorage, setLocalStorage, getSessionStorage, getCookie,
...
```typescript
function waitForSelector(selector: string, opts?: WaitForSelectorOptions): Promise<Element>;
```
...
Throws a `ToolError` with code `'json_parse_error'` if the response is not valid JSON.
